...
function renderProfileApproval(s) {
  ...
  (rec.categorySummaries || []).forEach(cs => {
    const pct = cs.maxScore > 0 ? Number((cs.score / cs.maxScore * 100).toFixed(0)) : 0;
    const level = pct >= 100 ? "high" : pct >= 50 ? "mid" : "low";
    const item = document.createElement("div");
    item.className = "score-item";
    item.innerHTML = `
      <div class="label">${esc(catLabel[cs.category] || cs.category)}</div>
      <div class="value">${cs.score} / ${cs.maxScore} | J:${cs.decisionScore ?? "-"} A:${cs.actionScore ?? "-"} | conf:${cs.avgConfidence ?? "-"}</div>
